...
-- Refactorisation de la table carte_resident et création d'une table de relation
-- car une carte resident peut appartenir à plusieurs passeports

-- Créer la nouvelle table de relation carte_resident_passeport AVANT de supprimer les colonnes
CREATE TABLE IF NOT EXISTS carte_resident_passeport (
    id SERIAL PRIMARY KEY,
    id_carte_resident INT NOT NULL,
    id_passeport INT NOT NULL,
    id_demande INT NOT NULL,
    date_association TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (id_carte_resident) REFERENCES carte_resident(id) ON DELETE CASCADE,
    FOREIGN KEY (id_passeport) REFERENCES passeport(id) ON DELETE CASCADE,
    FOREIGN KEY (id_demande) REFERENCES demande(id) ON DELETE CASCADE,
    UNIQUE(id_carte_resident, id_passeport, id_demande)
);

-- Migrer les données existantes de carte_resident vers carte_resident_passeport
-- (avant de supprimer les colonnes id_passeport et id_demande)
INSERT INTO carte_resident_passeport (id_carte_resident, id_passeport, id_demande, date_association)
SELECT id, id_passeport, id_demande, CURRENT_TIMESTAMP
FROM carte_resident
WHERE id_passeport IS NOT NULL AND id_demande IS NOT NULL
ON CONFLICT (id_carte_resident, id_passeport, id_demande) DO NOTHING;

-- Supprimer les contraintes de clés étrangères de la table carte_resident existante
ALTER TABLE carte_resident DROP CONSTRAINT IF EXISTS carte_resident_id_passeport_fkey;
ALTER TABLE carte_resident DROP CONSTRAINT IF EXISTS carte_resident_id_demande_fkey;

-- Supprimer les colonnes id_passeport et id_demande de la table carte_resident
ALTER TABLE carte_resident DROP COLUMN IF EXISTS id_passeport;
ALTER TABLE carte_resident DROP COLUMN IF EXISTS id_demande;

-- Créer les index pour améliorer les performances
CREATE INDEX IF NOT EXISTS idx_carte_resident_passeport_carte ON carte_resident_passeport(id_carte_resident);
CREATE INDEX IF NOT EXISTS idx_carte_resident_passeport_passeport ON carte_resident_passeport(id_passeport);
CREATE INDEX IF NOT EXISTS idx_carte_resident_passeport_demande ON carte_resident_passeport(id_demande);